...
double *compass_search (
        double function_handle ( int m, double x[],
            const vector<pCharUlong> &v,
            const string &seq, const vector<double> &qua, double lower[], double upper[], double sumBound ),
        int m, double x0[],
        const vector<pCharUlong> &v,
        const string &seq, const vector<double> &qua, double lower[], double upper[], double sumBound,
       ...
       //
{
    ...
    xd = new double[m];
    r8vec_copy ( m, x0, x );
    fx = function_handle ( m, x, v, seq, qua, lower, upper, sumBound );
    ...
    while ( k < k_max )
    {
        ...
        for ( ii = 1; ii <= 2 * m; ii++ )
        {
            r8vec_copy ( m, x, xd );
            xd[i] = xd[i] + s * delta;
            fxd = function_handle ( m, xd, v, seq, qua, lower, upper, sumBound );
            //
